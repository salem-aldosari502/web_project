...
function SendMessage() {
  ...
  async function fetchMessages() {
    try {
      ...
      setFeedback({
        type: 'error',
        text: `Could not load messages: ${err.message}`
      });
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.email.trim() || !formData.message.trim()) {
      setFeedback({
        type: 'error',
        text: 'Recipient email and message are required'
      });
    ...
    try {
      const res = await fetch(`${API}/api/messages/admin-send`, {
        method: 'POST',
        headers: {
  ...
  const saveEdit = async () => {
    ...
    try {
      const res = await fetch(`${API}/api/messages/${editing._id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          ...authHeader()
      ...
      <div className="ud-shell">
        ...
        <div className="ud-header">
          <div>
            ...
            <p>
              Send announcements, replies, or direct messages to users.
            </p>
          </div>
